...
class Hyperparameters:
    ...
    engram_bigram_buckets = int(os.environ.get("ENGRAM_BIGRAM_BUCKETS", 4096))
    engram_trigram_buckets = int(os.environ.get("ENGRAM_TRIGRAM_BUCKETS", 8192))
    engram_dim = int(os.environ.get("ENGRAM_DIM", 64))
    engram_heads = int(os.environ.get("ENGRAM_HEADS", 3))
    engram_gate_bias = float(os.environ.get("ENGRAM_GATE_BIAS", 1.0))
    engram_no_smear = bool(int(os.environ.get("ENGRAM_NO_SMEAR", "0")))
    engram_layers = os.environ.get("ENGRAM_LAYERS", "0")
...
# Hash primes for Engram — module-level constants.
_ENGRAM_BIGRAM_PRIMES = [(36313, 27191), (52711, 43397), (73291, 18457), (61403, 32771)]
_ENGRAM_TRIGRAM_PRIMES = [(36313, 27191, 15473), (52711, 43397, 61631), (73291, 18457, 41203), (61403, 32771, 54269)]

class EngramMemory(nn.Module):
    """Multi-order N-gram memory with multi-head hashing and context-aware gating.
    Deterministic hash-based lookup into learned embedding tables for bigrams and
    trigrams, with K independent hash heads per order. A sigmoid gate modulates
    how much memory signal to inject, conditioned on the current hidden state."""
    def __init__(self, bigram_buckets: int, trigram_buckets: int, engram_dim: int,
                 num_heads: int, model_dim: int, gate_bias: float = 1.0):
        super().__init__()
        self.bigram_buckets = bigram_buckets
        self.trigram_buckets = trigram_buckets
        self.num_heads = num_heads
        self.bigram_embed = nn.Embedding(bigram_buckets, engram_dim)
        self.trigram_embed = nn.Embedding(trigram_buckets, engram_dim)
        nn.init.zeros_(self.bigram_embed.weight)
        nn.init.zeros_(self.trigram_embed.weight)
        self.proj = CastedLinear(engram_dim, model_dim, bias=False)
        nn.init.zeros_(self.proj.weight)
        self.gate_proj = CastedLinear(model_dim, 1, bias=False)
        nn.init.zeros_(self.gate_proj.weight)
        # Gate bias: positive = gate starts open (memory flows through from step 1).
        self.gate_bias = nn.Parameter(torch.tensor(gate_bias, dtype=torch.float32))
        self.scale = nn.Parameter(torch.tensor(0.05, dtype=torch.float32))

    def _hash_bigrams(self, tokens: Tensor, head_idx: int) -> Tensor:
        t = tokens.to(torch.int32)
        p0, p1 = _ENGRAM_BIGRAM_PRIMES[head_idx % len(_ENGRAM_BIGRAM_PRIMES)]
        mod = self.bigram_buckets - 1
        out = torch.empty_like(t)
        out[..., 0] = mod
        out[..., 1:] = torch.bitwise_xor(p0 * t[..., 1:], p1 * t[..., :-1]) % mod
        return out.long()

    def _hash_trigrams(self, tokens: Tensor, head_idx: int) -> Tensor:
        t = tokens.to(torch.int32)
        p0, p1, p2 = _ENGRAM_TRIGRAM_PRIMES[head_idx % len(_ENGRAM_TRIGRAM_PRIMES)]
        mod = self.trigram_buckets - 1
        out = torch.empty_like(t)
        out[..., 0] = mod
        out[..., 1] = mod
        out[..., 2:] = (torch.bitwise_xor(torch.bitwise_xor(p0 * t[..., 2:], p1 * t[..., 1:-1]), p2 * t[..., :-2])) % mod
        return out.long()

    def forward(self, token_ids: Tensor, hidden: Tensor) -> Tensor:
        bigram_sum = sum(self.bigram_embed(self._hash_bigrams(token_ids, h)) for h in range(self.num_heads))
        trigram_sum = sum(self.trigram_embed(self._hash_trigrams(token_ids, h)) for h in range(self.num_heads))
        combined = (bigram_sum + trigram_sum) * (1.0 / (2 * self.num_heads))
        memory = self.proj(combined)
        gate = torch.sigmoid(self.gate_proj(hidden) + self.gate_bias.to(dtype=hidden.dtype))
        return memory * gate * self.scale.to(dtype=memory.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        engram_bigram_buckets: int = 4096,
        engram_trigram_buckets: int = 8192,
        engram_dim: int = 64,
        engram_heads: int = 3,
        engram_gate_bias: float = 1.0,
        engram_no_smear: bool = False,
        engram_layers: str = "0",
        xsa_last_n: int = 0,
        rope_dims: int = 0,
        ln_scale: bool = False,
        ...
        # Engram: shared tables, with per-injection-point gates.
        self.engram_layer_indices = [int(x) for x in engram_layers.split(",") if x.strip()] if engram_enabled else []
        if engram_enabled:
            self.engram = EngramMemory(engram_bigram_buckets, engram_trigram_buckets, engram_dim, engram_heads, model_dim, gate_bias=engram_gate_bias)
            # Extra gates for deep-layer injection (layer 0 uses engram's built-in gate).
            self.engram_deep_gates = nn.ModuleList()
            for idx in self.engram_layer_indices:
                if idx > 0:
                    gate = CastedLinear(model_dim, 1, bias=False)
                    nn.init.zeros_(gate.weight)
                    self.engram_deep_gates.append(gate)
        else:
            self.engram = None
            self.engram_deep_gates = nn.ModuleList()
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim) if bigram_vocab_size > 0 and not engram_enabled else None
        self.engram_no_smear = engram_no_smear
    ...
    def _engram_at_layer(self, layer_idx: int, token_ids: Tensor, x: Tensor) -> Tensor:
        """Inject Engram memory at a specific layer. Layer 0 uses built-in gate; deeper layers use separate gates."""
        if self.engram is None or layer_idx not in self.engram_layer_indices:
            return x
        if layer_idx == 0:
            return x + self.engram(token_ids, x)
        # Deep-layer injection: reuse shared tables, separate gate.
        deep_idx = sum(1 for li in self.engram_layer_indices if 0 < li <= layer_idx) - 1
        bigram_sum = sum(self.engram.bigram_embed(self.engram._hash_bigrams(token_ids, h)) for h in range(self.engram.num_heads))
        trigram_sum = sum(self.engram.trigram_embed(self.engram._hash_trigrams(token_ids, h)) for h in range(self.engram.num_heads))
        combined = (bigram_sum + trigram_sum) * (1.0 / (2 * self.engram.num_heads))
        memory = self.engram.proj(combined)
        gate = torch.sigmoid(self.engram_deep_gates[deep_idx](x) + self.engram.gate_bias.to(dtype=x.dtype))
        return x + memory * gate * self.engram.scale.to(dtype=memory.dtype)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self._engram_at_layer(0, input_ids, x)
        if not self.engram_no_smear:
            x = self.smear(x)
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ...
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            x = self._engram_at_layer(i + 1, input_ids, x)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
            x = self._engram_at_layer(bi + 1, input_ids, x)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name:
        return "embed"
    if name.startswith("engram.") or name.startswith("bigram."):
        return "embed"
    if ".mlp." in name:
        return "mlp"
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        engram_bigram_buckets=args.engram_bigram_buckets,
        engram_trigram_buckets=args.engram_trigram_buckets,
        engram_dim=args.engram_dim,
        engram_heads=args.engram_heads,
        engram_gate_bias=args.engram_gate_bias,
        engram_no_smear=args.engram_no_smear,
    ...
    if base_model.engram is not None:
        scalar_params.append(base_model.engram.scale)
        scalar_params.append(base_model.engram.gate_bias)
        scalar_params.append(base_model.engram.proj.weight)
        scalar_params.append(base_model.engram.gate_proj.weight)
        for dg in base_model.engram_deep_gates:
            scalar_params.append(dg.weight)
    ...
    if base_model.engram is not None:
        tok_params.append({"params": [base_model.engram.bigram_embed.weight], "lr": token_lr, "base_lr": token_lr})
        tok_params.append({"params": [base_model.engram.trigram_embed.weight], "lr": token_lr, "base_lr": token_lr})
    ...
    eval_model = GPT(
        ...
        engram_enabled=args.engram_enabled, engram_bigram_buckets=args.engram_bigram_buckets,
        engram_trigram_buckets=args.engram_trigram_buckets, engram_dim=args.engram_dim,
        engram_heads=args.engram_heads,
        engram_gate_bias=args.engram_gate_bias,
        engram_no_smear=args.engram_no_smear,
        engram_layers=args.engram_layers,
